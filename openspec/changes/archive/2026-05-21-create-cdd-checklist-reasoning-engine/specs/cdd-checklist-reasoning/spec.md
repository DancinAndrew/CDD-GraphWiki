...
本規格定義了合規檢核決策與推理引擎原型 (CDD Checklist Reasoning Engine) 的核心行為與合規合約。引擎旨在解析結構化客戶情境 (`CustomerContext`)，依據合規規則對其進行分級、判定風險觸發點、配對適用義務與衝突，產出 100% 與金標黃金數據集對齊的 `CDDChecklist`。
