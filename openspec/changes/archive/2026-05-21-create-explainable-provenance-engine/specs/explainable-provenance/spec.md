...
本規格定義了可解釋推理與合規溯源引擎 (Explainable Reasoner and Provenance Engine) 的核心行為與合規契約。引擎旨在對 CDD 決策檢核表的輸出項目進行深度溯源，回溯鏈結 `CustomerContext`、`Obligation`、`Clause` 與 `SourceDocument` 的多維關聯，產出結構化、一鍵可追溯且含有原始法理引述明文的「合規解釋鏈」，並可生成人類可讀的合規審計軌跡報告。
